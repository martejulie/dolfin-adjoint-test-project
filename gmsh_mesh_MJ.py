...
if __name__ == '__main__':
    ...
    if False:
        ...
        df.File('facet_f.xml') << facet_f
        df.File('point_f.xml') << point_f
        # TODO: save f in a loadable format


    mesh = Mesh('mesh.xml')
    facet_f = MeshFunction('size_t', mesh, 'facet_f.xml')
    point_f = MeshFunction('size_t', mesh, 'point_f.xml')
    ...
    estimate_M(p_data=f,
               V=V, 
               W=V, 
               bc=None, #[DirichletBC(V, Constant(p_ves), facet_f, 1)],
               alpha=Constant(1),
               data_norm=data_norm)
