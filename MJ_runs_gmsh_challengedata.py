...
if True:
    data = np.load('experimental_data/xyz_example_data_challenge.npz')
    x = data['x']
    y = data['y']
    z = data['z']
...
#    xcoor = np.mean(embed_points, axis=0) #data['vessel_xcoor']
#    ycoor = np.mean(embed_points, axis=1) #data['vessel_ycoor']
#    cx, cy = xcoor, ycoor 
...
#
#np.savez('results/gmsh/results_xyz_challenge_data_0_001.npz', p_noisy=p_noisy, p_opt=p_opt, M_opt=M_opt, x=x, y=y, Hx=Hx, Hy=Hy) 
